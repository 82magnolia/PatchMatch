...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_a", required=False, type=str)
    parser.add_argument("--img_b", required=False, type=str)
    parser.add_argument("--img_a_prime", required=False, type=str)
    parser.add_argument("--img_b_prime", required=False, type=str)
    parser.add_argument("--save_dir", default="./log/result", type=str)
    args = parser.parse_args()

    obj_num = "250"
    A_random = np.random.randint(56)
    B_random = np.random.randint(56)
    if A_random == B_random:
        B_random = np.random.randint(56)

    Object_Path = os.path.join("datasets", "ObjectFolder_touch")
    img = np.array(Image.open(os.path.join(Object_Path, obj_num, f"{A_random}_scale_50_normal.jpg")))
    ref = np.array(Image.open(os.path.join(Object_Path, obj_num, f"{B_random}_scale_50_normal.jpg")))
    img_prime = np.array(Image.open(os.path.join(Object_Path, obj_num, f"{A_random}_scale_50_sim.jpg")))
    ref_prime_gt = np.array(Image.open(os.path.join(Object_Path, obj_num, f"{B_random}_scale_50_sim.jpg")))
    ...
    print(f"Image synthesis : {A_random} to {B_random}")
